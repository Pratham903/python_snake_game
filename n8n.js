...
if (body?.repository_summary && body?.commit_summary) {
  ...
  const author = commit?.author ?? { name: 'N/A', email: 'N/A', username: 'N/A' };
  ...
  htmlContent = `
    ...
    <p><strong>Files Modified:</strong> ${modifiedFiles.join(', ') || 'None'}</p>
  ...
  htmlContent = `
    ...
    <p><strong>Issue Body:</strong> ${issue.body || 'No description provided'}</p>
    <p><strong>Author:</strong> ${user.login}</p>
  ...
  // This is the fallback for any other data structure that isn't recognized.
